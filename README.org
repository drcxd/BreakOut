...
Then on Windows, users could execute =./bin/BreakOut= to run the
program. On Linux, the shared libraries should be exposed to the
system. This can be done by

#+begin_src shell
  LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/path_to_project_root/slib
  export LD_LIBRARY_PATH
#+end_src

Then the program in =./bin/BreakOut= could be executed.
